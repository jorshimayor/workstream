...
@pytest.mark.parametrize(("source", "weak"), [
    ...
    ("try: pass\nfinally: values = (x for x in ())", False),
    ("for x in xs: pass\nelse:\n def f(): pass", False),
    ("match x:\n case 1: f = lambda: 1", False),
    ("match x:\n case 1 if all(y for y in ()): pass", False),
    ("import pytest as pa\np = Local()\nfor x in xs:\n p.skip()\n list((p := pa) for _ in (1,))", True),
    ("items[next(x for x in ())] += 1", False),
    ("items[(lambda: 1)()] += 1", False),
    ("import pytest as pa\np = Local()\ntry: raise TypeError()\nexcept ((p := pa), ValueError)[1]: pass\nexcept TypeError: p.skip()", True),
    ("import pytest\nfor p in (pytest for _ in (0,)): p.skip()", True),
    ("import pytest\n[p.skip() for p in (pytest for _ in (0,))]", True),
    ("import pytest\nfor p, q in ((pytest, Local()),): q.skip()", False),
    ("import pytest\np = Local()\nfor x in xs:\n p.skip()\n class C:\n  global p\n  p = pytest", True),
    ("while ready: pass\nelse: values = (x for x in ())", False),
    ("try:\n def f(): pass\n raise ExceptionGroup('x', [ValueError()])\nexcept* ValueError: pass", False),
    ("try: raise ExceptionGroup('x', [ValueError(), TypeError()])\nexcept* ValueError: f = lambda: 1\nexcept* TypeError: values = (x for x in ())", False),
    ("try: pass\nexcept* ValueError: pass\nelse:\n def f(): pass", False),
    ("try: raise ExceptionGroup('x', [ValueError()])\nexcept* ValueError: pass\nfinally: f = lambda: 1", False),
    ...
    ("import pytest\nq = Local()\nfor z in zs:\n q.skip()\n [(q := pytest) for x in (1,) for y in ()]", False),
    ("p = Local()\nfor x in xs:\n p.skip()\n class C:\n  global p\n  import pytest as p", True),
    ("import pytest\np = Local()\nfor x in xs:\n p.skip()\n class C:\n  global p\n  p += pytest", True),
    ("import pytest\nfor p in {x for x in (pytest,)}: p.skip()", True),
    ("import pytest\nfor p in {x: 1 for x in (pytest,)}: p.skip()", True),
    ("import pytest\nfor marker in (x.mark for x in (pytest,)): marker.skip", True),
    ("import unittest\nfor case in (u.TestCase() for u in (unittest,)): case.skipTest()", True),
    ("import pytest\nfor p, q in ((x, Local()) for x in (pytest,)): q.skip()", False),
    ("p = print\nfor x in xs:\n p(ValueError)\n class C:\n  global p\n  from pytest import raises as p", True),
    ...
    ("import pytest\nconsume((pytest.skip() for _ in (1,)))", False),
    ("import pytest\nconsume(value=(pytest.skip() for _ in (1,)))", False),
    ("import pytest\nconsume(*(pytest.skip() for _ in (1,)))", True),
    ("import pytest\nlist(pytest.skip() for _ in (1,))", True),
    ("import pytest\n[q for x in (1,) for y in {} if pytest.skip()]", False),
