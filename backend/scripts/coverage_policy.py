...
def config_floor(path: Path) -> Decimal:
    ...
    try:
        floor = Decimal(str(report["fail_under"])).quantize(Decimal("0.000001"))
...
def evidence_data(data: dict, expected_head: str) -> dict:
    ...
    require(data.get("alembic_head") == expected_head, "evidence_alembic_invalid")
    for key in ("python_version", "coverage_version", "pytest_cov_version"):
        require(isinstance(data.get(key), str) and data[key], "evidence_version_invalid")
...
def runner_metadata(path: Path, expected_tree: str, expected_head: str) -> dict:
    ...
    require(data.get("alembic_head") == expected_head, "metadata_alembic_mismatch")
    require(data.get("tree_sha") == expected_tree, "metadata_tree_mismatch")
...
def weak_python(path: Path) -> bool:
    try:
        ...
        return True
    blocked = {"skip", "skipif", "xfail"}
    return any(
        isinstance(node, ast.Attribute)
        and node.attr in blocked
        and ast.unparse(node.value) in {"pytest", "pytest.mark", "unittest"}
        for node in ast.walk(tree)
    )


def repo_path(path: str) -> str:
    if (REPO / path).exists() or path.startswith(".agent-loop/"):
        return path
    return f"backend/{path}" if (BACKEND / path).exists() else path


def validate_delta(base: str, max_lines: int, allowed: set[str]) -> None:
    raw_files = changed_files(base, "HEAD")
    files = [repo_path(path) for path in raw_files]
    require(all(path in allowed or path.startswith(".agent-loop/") for path in files), "scope_violation")
    _, _, raw_rows = numstat(base, "HEAD")
    rows = [(repo_path(path), add, delete) for path, add, delete in raw_rows]
    ...
    raw_tests = [path for path in raw_files if repo_path(path) in tests]
    deleted = [line for line in diff_text(base, "HEAD", raw_tests).splitlines() if line.startswith("-") and not line.startswith("---")]
