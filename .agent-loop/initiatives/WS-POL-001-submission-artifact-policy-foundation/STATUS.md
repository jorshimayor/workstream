...
contract evidence is ready for PR/external review and human approval.
Implementation remains inactive until human review approves the actor
identity/profile implementation boundary.

## Active Chunk

`WS-POL-001-11` PR/external review, then human contract approval.
...
| `WS-POL-001-11` | Internal review complete; PR pending | `codex/ws-pol-001-11-actor-identity-profile-contract` | - | Defines local Workstream actor identity and actor profile registries for verified Flow actors before the next live API drill. |

## Blockers

| Blocker | Owner | Next action |
|---|---|---|
| None | - | Open PR for `WS-POL-001-11`, wait for external review, then human review decides whether to merge the contract before implementation starts. |
