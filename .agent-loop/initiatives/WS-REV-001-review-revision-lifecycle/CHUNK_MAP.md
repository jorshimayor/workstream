...
|---|---|---:|---|---|
| `WS-REV-001-PLAN` | Review And Revision Lifecycle Planning | L0 | None | Active; AUTH PR #140 reconciliation in review |
...
| `WS-REV-001-02` | Locked Review Policy And Task Lifecycle Alignment | L1 | AUTH canonical actor foundation; merged AUTH-13 `TaskAssignment.contributor_id` and AUTH-14 `Submission.contributor_id` clean cuts, or a separately approved owner handoff before either edits those schemas; ART submission commitment contract stable; canonical rejected/cancelled lifecycle amendment; D6 behavior approved | Proposed |
...
| `WS-REV-001-06` | Atomic Claims, Release, Preference, And Timers | L1 | 05; merged `WS-AUTH-001-REV-CUSTODY` and `WS-AUTH-001-PREP`; generic AUTH-09E admission plus separately provisioned exact expiry identities/static rows; WS-CON ReviewLease ContributionPolicyVersion freeze participant; hidden manifest later gates `WS-AUTH-001-REV-06` | Proposed |
...
| `WS-REV-001-09A` | Revision Context Preparation And Resubmission | L1 | 08; ADR 0010 adopted; retired compensation-context field removal merged; AUTH-14 submission cutover; PREP; hidden manifest later gates `WS-AUTH-001-REV-09A` | Proposed |
...
| `WS-REV-001-11` | Admin Overrides, Reviewer-Revocation Recovery, And Reconciliation | L1 | 10; AUTH invalidation; `WS-AUTH-001-REV-REG` registered/planned; PREP; generic AUTH-09E plus separately provisioned exact invalidation/reconciliation identities/static rows; ART Operator recovery port; hidden manifest later gates `WS-AUTH-001-REV-11` | Proposed |
| `WS-REV-001-12` | Snapshot Projection, Notifications, And Observability | L1 | 11; ART projection port; outbox foundation; PREP; generic AUTH-09E plus separately provisioned exact artifact-reference/projection identities/static rows; hidden manifest later gates `WS-AUTH-001-REV-12` | Proposed |
...
| `WS-REV-001-13` | Coherent Public Release, Live API Drill, And Release Proof | L1 | 12A; `WS-AUTH-001-REV-CUSTODY`; exact `WS-AUTH-001-REV-05/06/07/08/09A/11/12`, `WS-AUTH-001-REV-LIFECYCLE`, and ART evidence actions active after hidden behavior; ART/CON/outbox live readiness | Proposed |
...
```text
AUTH canonical human plus AUTH-13/14 contributor-field contracts
...
Planning is approved and merged AUTH PR #140/AUTH-08/ART-02A2 contracts are reconciled.
