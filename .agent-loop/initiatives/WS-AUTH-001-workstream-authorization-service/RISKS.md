...
| A22 | Service authority becomes dynamic, duplicated, or confused with an external token subject | Internal service gains unintended human or artifact authority | Store one fixed `service_identity` on service ActorProfile, keep issuer/subject only on its identity link, and derive actions from one reviewed static matrix with no assignment rows | Seven-identity/eleven-action parity, unknown-service denial, planned-action inertness, and no service grant tests |
