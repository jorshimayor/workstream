...
an explicit ASCII whitespace set: space, tab, newline, carriage return, form
feed, and vertical tab. Migration `0026` uses the same expression for its
pre-DDL dirty-row refusal and installed constraints. Direct-write tests cover
all five lifecycle reason fields, and previous-head dirty-row cases prove tab
and newline padding stop the upgrade without changing revision, schema, or
data.
