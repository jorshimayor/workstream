# Chunk Contract: WS-AUTH-001-ART-06A — ART 06A Post-Submit Materialization Activation
...
Integrate the exact AUTH evaluator contract and activate only `artifact.post_submit.checker_input.materialize` after
...
merged `WS-ART-001-06A` hidden manifest, AUTH-09E materializer admission, and `WS-AUTH-001-ART-CUSTODY`.
...
.agent-loop/merge-intents/WS-AUTH-001-ART-06A.json
...
- Allow, wrong-grant, cross-project, inactive actor/link, stale authority,
  ...
  pass as applicable.
- The exact checker-run/binding facts are recomposed inside the committing transaction.
