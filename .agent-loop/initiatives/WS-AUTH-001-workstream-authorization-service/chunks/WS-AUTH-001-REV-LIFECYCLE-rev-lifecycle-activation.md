# Chunk Contract: WS-AUTH-001-REV-LIFECYCLE — REV Lifecycle Repair Action Activation
...
Integrate the exact AUTH evaluator contract and activate only `review.revision_context.repair`, `review.revision_context.legacy_close`, `review.revision_obligation.close`, and `review.lifecycle.activation.manage` after
...
merged exact REV hidden manifests, AUTH-PREP, and `WS-AUTH-001-REV-REG`.
...
.agent-loop/merge-intents/WS-AUTH-001-REV-LIFECYCLE.json
...
- Allow, wrong-grant, cross-project, inactive actor/link, stale authority,
  ...
  pass as applicable.
- This chunk stays blocked until the registration contract and all four feature manifests name exact principals, facts, guards, surfaces, transaction owners, and any required fixed services.
