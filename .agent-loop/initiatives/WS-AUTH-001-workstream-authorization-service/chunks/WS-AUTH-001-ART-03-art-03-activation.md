# Chunk Contract: WS-AUTH-001-ART-03 — ART 03 Guide Source Action Activation
...
Integrate the exact AUTH evaluator contract and activate only `artifact.guide_source.ingest`, `artifact.guide_source.read`, and `artifact.guide_source.binding.create` after
...
merged `WS-ART-001-03` hidden manifest, AUTH-09E for guide-reader/binding services, the exact human project authority contract, and `WS-AUTH-001-ART-CUSTODY`.
...
.agent-loop/merge-intents/WS-AUTH-001-ART-03.json
...
- Allow, wrong-grant, cross-project, inactive actor/link, stale authority,
  ...
  pass as applicable.
- Human ingest authority and fixed-service read/binding authority remain separate decisions and cannot imply each other.
