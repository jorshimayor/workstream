# Chunk Contract: WS-AUTH-001-REV-06 — REV 06 Claim Lease And Expiry Activation
...
Integrate the exact AUTH evaluator contract and activate only `review.claim`, `review.release`, `review.decline_preference`, `review.preference_expiry.run`, and `review.lease_expiry.run` after
...
merged `WS-REV-001-06` hidden manifest, reviewer grants, AUTH-PREP, `WS-AUTH-001-REV-CUSTODY`, and separately reviewed exact fixed-service identities/matrix rows for expiry commands.
...
.agent-loop/merge-intents/WS-AUTH-001-REV-06.json
...
- Allow, wrong-grant, cross-project, inactive actor/link, stale authority,
  ...
  pass as applicable.
- Human claim/release/preference authority and service expiry authority are isolated; reviewer revocation cannot remove submitter or adjudicator grants.
