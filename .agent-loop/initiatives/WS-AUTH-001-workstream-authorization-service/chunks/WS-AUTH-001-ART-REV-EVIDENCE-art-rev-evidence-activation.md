# Chunk Contract: WS-AUTH-001-ART-REV-EVIDENCE — Review Evidence Binding Action Activation
...
Integrate the exact AUTH evaluator contract and activate only `artifact.review_evidence.binding.create` after
...
merged `WS-ART-001-REV-EVIDENCE` hidden capability, merged REV evidence finalization boundary, AUTH-PREP, AUTH-09E, and `WS-AUTH-001-ART-REV-EVIDENCE-REG`.
...
.agent-loop/merge-intents/WS-AUTH-001-ART-REV-EVIDENCE.json
...
- Allow, wrong-grant, cross-project, inactive actor/link, stale authority,
  ...
  pass as applicable.
- The activation proves the separate human and binding-service decisions, two evidence records, exact lock order, one transaction owner, and no authority substitution.
