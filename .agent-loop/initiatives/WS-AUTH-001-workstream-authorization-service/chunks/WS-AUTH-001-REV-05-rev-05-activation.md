# Chunk Contract: WS-AUTH-001-REV-05 — REV 05 Queue Read Activation
...
Integrate the exact AUTH evaluator contract and activate only `review.queue.read` and `review.queue.inspect` after
...
merged `WS-REV-001-05` hidden manifest, independent reviewer grants from AUTH-10, project visibility from AUTH-11, and `WS-AUTH-001-REV-CUSTODY`.
...
.agent-loop/merge-intents/WS-AUTH-001-REV-05.json
...
- Allow, wrong-grant, cross-project, inactive actor/link, stale authority,
  ...
  pass as applicable.
- Queue filtering occurs before counts/cursors and never synthesizes a combined contributor role.
