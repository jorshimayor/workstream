# Chunk Contract: WS-AUTH-001-ART-06B — ART 06B Checker Output Action Activation
...
Integrate the exact AUTH evaluator contract and activate only `artifact.checker_output.write` and `artifact.checker_output.binding.create` after
...
merged `WS-ART-001-06B` hidden manifest, AUTH-09E checker-output/binding service admission, and `WS-AUTH-001-ART-CUSTODY`.
...
.agent-loop/merge-intents/WS-AUTH-001-ART-06B.json
...
- Allow, wrong-grant, cross-project, inactive actor/link, stale authority,
  ...
  pass as applicable.
- Write and binding use their exact service rows and checker-run guards; neither action implies the other.
