# Chunk Contract: WS-AUTH-001-REV-11 — REV 11 Recovery And Reconciliation Activation
...
Integrate the exact AUTH evaluator contract and activate only `review.lease.force_release`, `review.queue.routing.override`, `review.queue.routing.correct`, `review.queue.close`, and `review.reconcile.run` after
...
merged `WS-REV-001-11` hidden manifest, applicable Operator authority, AUTH-PREP, `WS-AUTH-001-REV-CUSTODY`, and separately reviewed exact fixed-service identity/matrix rows for reconciliation.
...
.agent-loop/merge-intents/WS-AUTH-001-REV-11.json
...
- Allow, wrong-grant, cross-project, inactive actor/link, stale authority,
  ...
  pass as applicable.
- Operator recovery and service reconciliation are distinct candidates; reason, concealment, idempotency, and rollback evidence are mandatory.
