# Chunk Contract: WS-AUTH-001-REV-09A — REV 09A Finding Response Evidence Activation
...
Integrate the exact AUTH evaluator contract and activate only `review.finding_response_evidence.ingest` after
...
merged `WS-REV-001-09A` hidden manifest, exact submitter authority and revision obligation, AUTH-PREP, required ART evidence capability, and `WS-AUTH-001-REV-CUSTODY`.
...
.agent-loop/merge-intents/WS-AUTH-001-REV-09A.json
...
- Allow, wrong-grant, cross-project, inactive actor/link, stale authority,
  ...
  pass as applicable.
- The action reuses `submission.create` permission without becoming submission creation or allowing reviewer/adjudicator/admin substitution.
