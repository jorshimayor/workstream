# Chunk Contract: WS-AUTH-001-ART-04B — ART 04B Pre-Submit Materialization Activation
...
Integrate the exact AUTH evaluator contract and activate only `artifact.pre_submit.checker_input.materialize` after
...
merged `WS-ART-001-04B` hidden manifest, AUTH-09E materializer admission, and `WS-AUTH-001-ART-CUSTODY`.
...
.agent-loop/merge-intents/WS-AUTH-001-ART-04B.json
...
- Allow, wrong-grant, cross-project, inactive actor/link, stale authority,
  ...
  pass as applicable.
- The materializer can execute only this exact action against the sealed upload/task resource contract.
