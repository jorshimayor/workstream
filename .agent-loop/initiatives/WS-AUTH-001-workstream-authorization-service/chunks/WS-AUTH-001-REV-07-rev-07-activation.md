# Chunk Contract: WS-AUTH-001-REV-07 — REV 07 Context Chain And Finding Evidence Activation
...
Integrate the exact AUTH evaluator contract and activate only `review.context.read`, `review.chain.read`, and `review.finding_evidence.ingest` after
...
merged `WS-REV-001-07` hidden manifest, reviewer grants, AUTH-PREP for evidence mutation, required ART packet/evidence capability, and `WS-AUTH-001-REV-CUSTODY`.
...
.agent-loop/merge-intents/WS-AUTH-001-REV-07.json
...
- Allow, wrong-grant, cross-project, inactive actor/link, stale authority,
  ...
  pass as applicable.
- Lease-scoped packet visibility, no-self-review, evidence candidate/finalize, and rollback proof are mandatory.
