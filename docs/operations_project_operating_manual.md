...
- review policy
- revision policy
- compensation policy and published versions
- review guard
- lessons learned log
...
- active published CompensationPolicyVersion configured
...
- active published CompensationPolicyVersion available before assignment or
...
activated as separate guide-policy context. Compensation policy is project-level
...
`CompensationPolicyVersion` with exactly one explicit compensated/unpaid rule
...
the active published CompensationPolicyVersion.
...
- compensation policy has explicit submitter and reviewer rules before
...
- submission artifact requirements are clear
- an active submitter `CompensationPolicyVersion` is published; its rule may be
  explicitly unpaid
